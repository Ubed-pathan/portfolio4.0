...
export default function Header() {
  ...
  useEffect(() => {
    ...
    // Enhanced mobile scroll behavior fix
    if (isMobile) {
    ...
    >
      ...
      `}>
        <div className="flex justify-between items-center">
          {/* Logo - Remove hover animation on mobile */}
          <motion.div
            {...(!isMobile && {
              whileHover: { 
                scale: 1.05,
          ...
          {/* Desktop Navigation - Remove complex animations on mobile */}
          <div className="hidden md:flex space-x-2">
            {navItems.map((item, index) => (
              ...
              >
                {/* Simplified desktop effects only */}
          ...
          <div className="flex items-center space-x-4">
            {/* Theme Toggle - Simplified mobile animations */}
            <motion.button
              whileHover={!isMobile ? { 
                scale: 1.15,
              ...
              whileTap={{ scale: 0.95 }}
              onClick={toggleTheme}
              className={`p-3 glass rounded-full transition-all duration-300 relative overflow-hidden
                ${theme === 'dark' 
              ...
              `}
            >
              {/* Remove all desktop effects on mobile */}
              {!isMobile && (
                <>
                  <div 
          ...
          >
            {navItems.map((item, index) => (
              <motion.a
                ...
                className="block py-3 px-4 rounded-xl font-semibold text-lg uppercase tracking-wide transition-all duration-200 relative group"
              >
                {/* Mobile item glow */}
